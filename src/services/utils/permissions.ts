import type { User, Thread, QNAThread, Comment } from "@/features/forum/types";
...
export const canMarkAsAnswer = (user: User | null, thread: Thread | QNAThread): boolean => {
  if (!user || !isQNA(thread)) return false;
  return user.id === thread.creator.id;
};

export const canCreateContent = (user: User | null): boolean => user !== null;
